...
export default function FondList() {
  ...
  const items = useRecoilValue(storeGetAllFond);
  const getFonds = items.items;

  const [TABLE_ROWS, setTableRows] = useState(getFonds);
  ...
  const handleChange = (e) => {
    ...
    const searchFonds = getFonds.filter((user) => {
      if (user.nom.includes(e.target.value)) return user;
    });

    setTableRows(searchFonds);
  };
          ...
          <div className="mb-8 flex items-center justify-between gap-8">
            <div>
              ...
              <Typography color="gray" className="mt-1 font-normal">
                Voir les informations sur les différents Fonds
              </Typography>
