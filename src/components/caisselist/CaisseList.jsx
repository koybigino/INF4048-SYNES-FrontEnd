...
export default function CaisseList() {
	...
	const items = useRecoilValue(storeGetAllCaisse)
	const getCaisse = items.items

	const [TABLE_ROWS, setTableRows] = useState(getCaisse)
	...
	const handleChange = (e) => {
		...
		const searchCaisse = getCaisse.filter((user) => {
			if (user.nom.includes(e.target.value)) return user
		})

		setTableRows(searchCaisse)
	}
				...
				>
					<div className='mb-8 flex items-center justify-between gap-8'>
						<div>
							...
							>
								Voir les informations sur les différents Caisse
							</Typography>
